...
public static class ClientInspectionDiagnostics4HundredStageSmoke
{
    public static async Task RunAsync(Action<bool,string> Check)
    {
        ...
        for(var i0=0;i0<10;i0++)
        {
            round++;
                ...
                {
                    Roi=snapshot.Roi is null
                        ? null
                        : snapshot.Roi with { ProductionSessionId=Guid.NewGuid() }
                };
                var diagnostic=ClientInspectionDiagnosticsRuntime.Analyze(tampered);
                Check(!diagnostic.IsCoherent &&
                      diagnostic.Errors.Any(error=>error.Contains("ROI and replay",StringComparison.Ordinal)),
                      "cross-layer diagnostic should reject ROI/session identity drift");
        }
