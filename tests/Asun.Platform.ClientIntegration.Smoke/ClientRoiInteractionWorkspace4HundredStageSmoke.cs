...
public static class ClientRoiInteractionWorkspace4HundredStageSmoke
{
    public static async Task RunAsync(Action<bool,string> Check)
    {
        ...
        for(var i0=0;i0<10;i0++)
        {
            round++;
                ...
                roi.Submit(ViewportInputEventKind.PointerDown,new Vector2(120,120));
                roi.Submit(ViewportInputEventKind.PointerUp,new Vector2(180,180));
                var beforeReset=roi.Capture();
                roi.StopInteraction();
                var stopped=roi.State==ViewportPresentationState.Stopped;
                Check(beforeReset.RoiCount==1 &&
                      stopped,
                      "ROI interaction stop should end the input lifecycle without losing the captured ROI projection");
        }
