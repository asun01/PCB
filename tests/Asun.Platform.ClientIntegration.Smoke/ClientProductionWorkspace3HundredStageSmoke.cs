...
public static class ClientProductionWorkspace3HundredStageSmoke
{
    public static async Task RunAsync(Action<bool,string> Check)
    {
        ...
        for(var i0=0;i0<10;i0++)
        {
            round++;
                ...
                try
                {
                    await workspace.StartAsync(new Asun.Device.Impl.SimulatedFrameSource(8,8),cancellation.Token);
                }
                catch(OperationCanceledException)
                {
                }
                Check(workspace.Snapshot.Status==ClientExecutionStatus.Cancelled,
                      "client cancellation should become a client projection state");
        }
