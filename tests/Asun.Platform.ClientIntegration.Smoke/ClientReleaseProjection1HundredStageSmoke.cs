...
public static class ClientReleaseProjection1HundredStageSmoke
{
    public static async Task RunAsync(Action<bool,string> Check)
    {
        ...
        for(var i0=0;i0<10;i0++)
        {
            round++;
                ...
                var projection=ClientReleaseProjectionRuntime.Create(replay,manifest);
                Check(ClientReleaseProjectionRuntime.IsValid(replay,manifest) &&
                      projection.ProgramId==definition.ProgramPlan.ProgramId &&
                      projection.ProductionSessionId==report.SessionId &&
                      projection.ReleaseReady &&
                      projection.ArtifactPath=="simulation/release.pcb" &&
                      projection.ProjectionFingerprint.Length==64,
                      "completed client result should project canonical logical Release readiness");
        }
