...
public static class ClientProductionProgress4HundredStageSmoke
{
    ...
    public static async Task RunAsync(Action<bool,string> Check)
    {
        ...
        for(var iteration0=0;iteration0<10;iteration0++)
        {
            ...
            var workspace=new ClientProductionWorkspace(new ImmediateProgressRunner());
            workspace.Load(definition);
            await workspace.StartAsync(new Asun.Device.Impl.SimulatedFrameSource(8,8));
            var snapshot=workspace.Snapshot;
            Check(snapshot.Status==ClientExecutionStatus.Completed &&
                  snapshot.TargetFrameCount==3 &&
                  snapshot.FramesProcessed==3 &&
                  snapshot.LastFrameCount==3 &&
                  snapshot.LastSequence?.Value==3,
                  "completed client session must publish terminal progress");
        }
