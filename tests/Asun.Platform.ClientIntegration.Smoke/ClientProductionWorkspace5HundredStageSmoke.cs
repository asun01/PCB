...
public static class ClientProductionWorkspace5HundredStageSmoke
{
    public static async Task RunAsync(Action<bool,string> Check)
    {
        ...
        for(var i0=0;i0<10;i0++)
        {
            round++;
                ...
                workspace.Reset();
                Check(workspace.Snapshot.Status==ClientExecutionStatus.Idle &&
                      workspace.Snapshot.ProgramId is null &&
                      workspace.Snapshot.ActiveSessionId is null &&
                      workspace.Snapshot.LastFrameCount==0 &&
                      workspace.Snapshot.LastReportFingerprint is null,
                      "client reset should clear only the application projection and active definition");
        }
