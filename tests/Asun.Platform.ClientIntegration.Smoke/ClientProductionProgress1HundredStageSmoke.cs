...
public static class ClientProductionProgress1HundredStageSmoke
{
    ...
    public static async Task RunAsync(Action<bool,string> Check)
    {
        ...
        for(var iteration0=0;iteration0<10;iteration0++)
        {
            ...
            var workspace=new ClientProductionWorkspace();
            workspace.Load(definition);
            var snapshot=workspace.Snapshot;
            Check(snapshot.Status==ClientExecutionStatus.Ready &&
                  snapshot.TargetFrameCount==3 &&
                  snapshot.FramesProcessed==0,
                  "loaded client session must expose target frame count before execution");
        }
