...
public static class ProductionCaptureSessionReconciliation2HundredStageSmoke
{
    ...
    Enumerable.Range(1,3).Select(i=>"capture-"+i.ToString()).ToArray());
                ...
                fingerprints[i1%3]="tampered-"+round;
    ...
    Enumerable.Range(1,3).Select(i=>"capture-"+i.ToString()).ToArray());
                var fingerprints=c.Fingerprints.ToArray();
                fingerprints[i2%3]="tampered-"+round;
                var tampered=new CaptureSessionSnapshot(c.CapturedCount,c.FirstSequence,c.LastSequence,fingerprints);
                Check(!ProductionCaptureSessionReconciliationRuntime.IsValid(r,tampered) &&
                      (group2<9 || round==100),
                      "payload fingerprint drift must be rejected");
    ...
    Enumerable.Range(1,3).Select(i=>"capture-"+i.ToString()).ToArray());
                var fingerprints=c.Fingerprints.ToArray();
                fingerprints[i3%3]="tampered-"+round;
                var tampered=new CaptureSessionSnapshot(c.CapturedCount,c.FirstSequence,c.LastSequence,fingerprints);
                Check(!ProductionCaptureSessionReconciliationRuntime.IsValid(r,tampered) &&
                      (group3<9 || round==100),
                      "payload fingerprint drift must be rejected");
    ...
    Enumerable.Range(1,3).Select(i=>"capture-"+i.ToString()).ToArray());
                var fingerprints=c.Fingerprints.ToArray();
                fingerprints[i4%3]="tampered-"+round;
                var tampered=new CaptureSessionSnapshot(c.CapturedCount,c.FirstSequence,c.LastSequence,fingerprints);
                Check(!ProductionCaptureSessionReconciliationRuntime.IsValid(r,tampered) &&
                      (group4<9 || round==100),
                      "payload fingerprint drift must be rejected");
    ...
    Enumerable.Range(1,3).Select(i=>"capture-"+i.ToString()).ToArray());
                var fingerprints=c.Fingerprints.ToArray();
                fingerprints[i5%3]="tampered-"+round;
                var tampered=new CaptureSessionSnapshot(c.CapturedCount,c.FirstSequence,c.LastSequence,fingerprints);
                Check(!ProductionCaptureSessionReconciliationRuntime.IsValid(r,tampered) &&
                      (group5<9 || round==100),
                      "payload fingerprint drift must be rejected");
    ...
    Enumerable.Range(1,3).Select(i=>"capture-"+i.ToString()).ToArray());
                var fingerprints=c.Fingerprints.ToArray();
                fingerprints[i6%3]="tampered-"+round;
                var tampered=new CaptureSessionSnapshot(c.CapturedCount,c.FirstSequence,c.LastSequence,fingerprints);
                Check(!ProductionCaptureSessionReconciliationRuntime.IsValid(r,tampered) &&
                      (group6<9 || round==100),
                      "payload fingerprint drift must be rejected");
    ...
    Enumerable.Range(1,3).Select(i=>"capture-"+i.ToString()).ToArray());
                var fingerprints=c.Fingerprints.ToArray();
                fingerprints[i7%3]="tampered-"+round;
                var tampered=new CaptureSessionSnapshot(c.CapturedCount,c.FirstSequence,c.LastSequence,fingerprints);
                Check(!ProductionCaptureSessionReconciliationRuntime.IsValid(r,tampered) &&
                      (group7<9 || round==100),
                      "payload fingerprint drift must be rejected");
    ...
    Enumerable.Range(1,3).Select(i=>"capture-"+i.ToString()).ToArray());
                var fingerprints=c.Fingerprints.ToArray();
                fingerprints[i8%3]="tampered-"+round;
                var tampered=new CaptureSessionSnapshot(c.CapturedCount,c.FirstSequence,c.LastSequence,fingerprints);
                Check(!ProductionCaptureSessionReconciliationRuntime.IsValid(r,tampered) &&
                      (group8<9 || round==100),
                      "payload fingerprint drift must be rejected");
    ...
    Enumerable.Range(1,3).Select(i=>"capture-"+i.ToString()).ToArray());
                var fingerprints=c.Fingerprints.ToArray();
                fingerprints[i9%3]="tampered-"+round;
                var tampered=new CaptureSessionSnapshot(c.CapturedCount,c.FirstSequence,c.LastSequence,fingerprints);
                Check(!ProductionCaptureSessionReconciliationRuntime.IsValid(r,tampered) &&
                      (group9<9 || round==100),
                      "payload fingerprint drift must be rejected");
    ...
    Enumerable.Range(1,3).Select(i=>"capture-"+i.ToString()).ToArray());
                var fingerprints=c.Fingerprints.ToArray();
                fingerprints[i10%3]="tampered-"+round;
                var tampered=new CaptureSessionSnapshot(c.CapturedCount,c.FirstSequence,c.LastSequence,fingerprints);
                Check(!ProductionCaptureSessionReconciliationRuntime.IsValid(r,tampered) &&
                      (group10<9 || round==100),
                      "payload fingerprint drift must be rejected");
