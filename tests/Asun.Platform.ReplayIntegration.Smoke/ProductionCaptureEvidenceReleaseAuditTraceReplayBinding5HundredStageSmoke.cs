...
public static class ProductionCaptureEvidenceReleaseAuditTraceReplayBinding5HundredStageSmoke
{
...
static PcbEvidenceReleaseAuditTrace AuditTrace(string manifest)
{
    ...
    public static Task RunAsync(Action<bool,string> Check)
    {
        ...
        {
            round++;
            var replay=Replay(); var trace=AuditTrace(replay.ReleaseManifestFingerprint); var left=ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.Create(replay,trace); var right=ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.Create(replay,trace); Check(ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.IsEquivalent(left,right) && left.AuditTraceFingerprint==right.AuditTraceFingerprint && (group0<9 || round==100),"equivalent audit replay bindings must converge");
        }
        for(var group1=0;group1<10;group1++)
        {
            round++;
            var replay=Replay(); var trace=AuditTrace(replay.ReleaseManifestFingerprint); var left=ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.Create(replay,trace); var right=ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.Create(replay,trace); Check(ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.IsEquivalent(left,right) && left.AuditTraceFingerprint==right.AuditTraceFingerprint && (group1<9 || round==100),"equivalent audit replay bindings must converge");
        }
        for(var group2=0;group2<10;group2++)
        {
            round++;
            var replay=Replay(); var trace=AuditTrace(replay.ReleaseManifestFingerprint); var left=ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.Create(replay,trace); var right=ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.Create(replay,trace); Check(ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.IsEquivalent(left,right) && left.AuditTraceFingerprint==right.AuditTraceFingerprint && (group2<9 || round==100),"equivalent audit replay bindings must converge");
        }
        for(var group3=0;group3<10;group3++)
        {
            round++;
            var replay=Replay(); var trace=AuditTrace(replay.ReleaseManifestFingerprint); var left=ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.Create(replay,trace); var right=ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.Create(replay,trace); Check(ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.IsEquivalent(left,right) && left.AuditTraceFingerprint==right.AuditTraceFingerprint && (group3<9 || round==100),"equivalent audit replay bindings must converge");
        }
        for(var group4=0;group4<10;group4++)
        {
            round++;
            var replay=Replay(); var trace=AuditTrace(replay.ReleaseManifestFingerprint); var left=ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.Create(replay,trace); var right=ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.Create(replay,trace); Check(ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.IsEquivalent(left,right) && left.AuditTraceFingerprint==right.AuditTraceFingerprint && (group4<9 || round==100),"equivalent audit replay bindings must converge");
        }
        for(var group5=0;group5<10;group5++)
        {
            round++;
            var replay=Replay(); var trace=AuditTrace(replay.ReleaseManifestFingerprint); var left=ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.Create(replay,trace); var right=ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.Create(replay,trace); Check(ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.IsEquivalent(left,right) && left.AuditTraceFingerprint==right.AuditTraceFingerprint && (group5<9 || round==100),"equivalent audit replay bindings must converge");
        }
        for(var group6=0;group6<10;group6++)
        {
            round++;
            var replay=Replay(); var trace=AuditTrace(replay.ReleaseManifestFingerprint); var left=ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.Create(replay,trace); var right=ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.Create(replay,trace); Check(ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.IsEquivalent(left,right) && left.AuditTraceFingerprint==right.AuditTraceFingerprint && (group6<9 || round==100),"equivalent audit replay bindings must converge");
        }
        for(var group7=0;group7<10;group7++)
        {
            round++;
            var replay=Replay(); var trace=AuditTrace(replay.ReleaseManifestFingerprint); var left=ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.Create(replay,trace); var right=ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.Create(replay,trace); Check(ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.IsEquivalent(left,right) && left.AuditTraceFingerprint==right.AuditTraceFingerprint && (group7<9 || round==100),"equivalent audit replay bindings must converge");
        }
        for(var group8=0;group8<10;group8++)
        {
            round++;
            var replay=Replay(); var trace=AuditTrace(replay.ReleaseManifestFingerprint); var left=ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.Create(replay,trace); var right=ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.Create(replay,trace); Check(ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.IsEquivalent(left,right) && left.AuditTraceFingerprint==right.AuditTraceFingerprint && (group8<9 || round==100),"equivalent audit replay bindings must converge");
        }
        for(var group9=0;group9<10;group9++)
        {
            round++;
            var replay=Replay(); var trace=AuditTrace(replay.ReleaseManifestFingerprint); var left=ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.Create(replay,trace); var right=ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.Create(replay,trace); Check(ProductionCaptureEvidenceReleaseAuditTraceReplayBindingRuntime.IsEquivalent(left,right) && left.AuditTraceFingerprint==right.AuditTraceFingerprint && (group9<9 || round==100),"equivalent audit replay bindings must converge");
        }
