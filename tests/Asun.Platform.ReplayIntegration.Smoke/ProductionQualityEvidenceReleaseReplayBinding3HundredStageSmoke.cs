...
public static class ProductionQualityEvidenceReleaseReplayBinding3HundredStageSmoke
{
    public static async ValueTask RunAsync(Action<bool,string> assert)
    {
        ...
        var badSession=binding with {ProductionSessionId=Guid.NewGuid()};
var badQuality=binding with {QualityRunId=Guid.NewGuid()};
for(var i=0;i<10;i++) Check(!ProductionQualityEvidenceReleaseReplayBindingRuntime.IsValid(definition,production,qualityRun,evidence,bundle,manifest,badSession),"Session tampering should be rejected.");
for(var i=0;i<10;i++) Check(!ProductionQualityEvidenceReleaseReplayBindingRuntime.IsValid(definition,production,qualityRun,evidence,bundle,manifest,badQuality),"Quality identity tampering should be rejected.");
for(var i=0;i<10;i++) Check(ProductionQualityEvidenceReleaseReplayBindingRuntime.Validate(definition,production,qualityRun,evidence,bundle,manifest,badSession).Count>0,"Session tampering should produce diagnostics.");
for(var i=0;i<10;i++) Check(ProductionQualityEvidenceReleaseReplayBindingRuntime.Validate(definition,production,qualityRun,evidence,bundle,manifest,badQuality).Count>0,"Quality tampering should produce diagnostics.");
for(var i=0;i<10;i++) Check(binding.ProductionSessionId==production.SessionId,"Baseline session identity should remain stable.");
for(var i=0;i<10;i++) Check(binding.QualityRunId==qualityRun.RunId,"Baseline Quality identity should remain stable.");
