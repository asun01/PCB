...
public static class ProductionQualityEvidenceReleaseReplayBinding1HundredStageSmoke
{
    public static async ValueTask RunAsync(Action<bool,string> assert)
    {
        ...
        for(var i=0;i<10;i++) Check(binding.ProductionSessionId==production.SessionId,"Binding should preserve Production session.");
for(var i=0;i<10;i++) Check(binding.QualityRunId==qualityRun.RunId,"Binding should preserve Quality run.");
for(var i=0;i<10;i++) Check(binding.ReplayBundleFingerprint==bundle.Fingerprint,"Binding should preserve replay bundle identity.");
for(var i=0;i<10;i++) Check(binding.ReleaseManifestFingerprint==manifest.Fingerprint,"Binding should preserve Release manifest identity.");
for(var i=0;i<10;i++) Check(binding.ReleaseReady==ReleaseReadinessRuntime.Evaluate(manifest).Ready,"Binding should preserve factual release readiness.");
for(var i=0;i<10;i++) Check(binding.Fingerprint.Length==64,"Binding fingerprint should be fixed width.");
for(var i=0;i<10;i++) Check(ProductionQualityEvidenceReleaseReplayBindingRuntime.IsValid(definition,production,qualityRun,evidence,bundle,manifest,binding),"Canonical release replay binding should validate.");
for(var i=0;i<10;i++) Check(ProductionQualityEvidenceReleaseReplayBindingRuntime.Create(definition,production,qualityRun,evidence,bundle,manifest).Fingerprint==binding.Fingerprint,"Binding creation should be deterministic.");
for(var i=0;i<10;i++) Check(binding.Fingerprint.All(Uri.IsHexDigit),"Binding fingerprint should be hexadecimal.");
for(var i=0;i<10;i++) Check(ReleaseManifestValidationRuntime.IsValid(manifest),"Source release manifest should remain valid.");
        assert(round==100,$"ProductionQualityEvidenceReleaseReplayBinding1HundredStageSmoke should execute exactly 100 numbered rounds; actual {round}.");
    }
}
