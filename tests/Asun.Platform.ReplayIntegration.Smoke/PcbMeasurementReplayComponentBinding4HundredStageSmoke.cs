...
public static class PcbMeasurementReplayComponentBinding4HundredStageSmoke
{
    public static Task RunAsync(Action<bool,string> Check)
    {
        ...
        for(var group1=0;group1<10;group1++)
        {
            for(var i1=0;i1<10;i1++)
            {
                ...
                var descriptors=i1%2==0
                    ? fixture.ReplayDescriptors.Take(1).ToArray()
                    : fixture.ReplayDescriptors.Select((item,index)=>index==1 ? item with { ReleaseReady=false } : item).ToArray();
                Check(
                    !PcbMeasurementReplayComponentBindingRuntime.IsValid(
                        fixture.ExecutionBinding,fixture.MeasurementBindings,descriptors) &&
                    (group1<9 || round==100),
                    "replay count/readiness drift must be rejected");
            }
        }
