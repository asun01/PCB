...
var badBindingFingerprint=replayBinding with {Fingerprint=new string('b',64)};
...
for(var i=0;i<10;i++) Check(!ProductionMeasurementQualityEvidenceReleaseReplayDescriptorRuntime.IsValid(measurementBinding,badBindingFingerprint,descriptor),"Replay binding fingerprint tampering should be rejected.");
...
for(var i=0;i<10;i++) Check(ProductionMeasurementQualityEvidenceReleaseReplayDescriptorRuntime.Validate(measurementBinding,badBindingFingerprint,descriptor).Count>0,"Replay binding tampering should emit diagnostics.");
for(var i=0;i<10;i++) Check(descriptor.ReplayBundleFingerprint==replayBinding.ReplayBundleFingerprint,"Baseline replay bundle identity remains stable.");
for(var i=0;i<10;i++) Check(descriptor.ReleaseReplayBindingFingerprint==replayBinding.Fingerprint,"Baseline replay binding identity remains stable.");
for(var i=0;i<10;i++) Check(descriptor.ProductionSessionId==replayBinding.ProductionSessionId,"Production session remains stable.");
for(var i=0;i<10;i++) Check(descriptor.QualityRunId==replayBinding.QualityRunId,"Quality run remains stable.");
...
for(var i=0;i<10;i++) Check(descriptor.DescriptorFingerprint==ProductionMeasurementQualityEvidenceReleaseReplayDescriptorRuntime.Create(measurementBinding,replayBinding).DescriptorFingerprint,"Replay creation remains deterministic.");
