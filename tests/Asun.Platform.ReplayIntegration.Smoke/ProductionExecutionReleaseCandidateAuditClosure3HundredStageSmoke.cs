...
public static class ProductionExecutionReleaseCandidateAuditClosure3HundredStageSmoke
{
    public static async ValueTask RunAsync(Action<bool,string> assert)
    {
        ...
        var badSession=production with {SessionId=Guid.NewGuid()};
var badAudit=auditClosure with {ProductionSessionId=Guid.NewGuid()};
for(var i=0;i<10;i++) Check(!ProductionExecutionReleaseCandidateAuditClosureRuntime.IsValid(auditClosure,manifest,definition,badSession,closure),"Production report session tampering should be rejected.");
for(var i=0;i<10;i++) Check(!ProductionExecutionReleaseCandidateAuditClosureRuntime.IsValid(badAudit,manifest,definition,production,closure),"Audit session tampering should be rejected.");
for(var i=0;i<10;i++) Check(ProductionExecutionReleaseCandidateAuditClosureRuntime.Validate(auditClosure,manifest,definition,badSession,closure).Count>0,"Production report tampering should emit diagnostics.");
...
for(var i=0;i<10;i++) Check(ProductionExecutionReleaseCandidateAuditClosureRuntime.IsValid(auditClosure,manifest,definition,production,closure),"Baseline audit remains valid.");
        ...
        assert(round==100,$"ProductionExecutionReleaseCandidateAuditClosure3HundredStageSmoke should execute exactly 100 numbered rounds; actual {round}.");
    }
}
