    ...
    public static Task RunAsync(Action<bool,string> Check)
    {
        ...
        for(var group=0;group<10;group++)
        {
            for(var i=0;i<10;i++)
            {
                ...
                refs[i%3]=refs[i%3] with { PayloadFingerprint="ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff" };
                var valid=ProductionCaptureEvidenceReplayBindingRuntime.Validate(report,refs).Count==0;
                Check(!valid && (group<9 || round==100),"matrix");
            }
        }
