    ...
    public static Task RunAsync(Action<bool,string> Check)
    {
        ...
        for(var group0=0;group0<1;group0++)
        {
            round++;
                ...
                refs[0%3]=refs[0%3] with { PayloadFingerprint="ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff" };
                Check(ProductionCaptureEvidenceReplayBindingRuntime.Validate(report,refs).Count>0 &&
                      (round==100),"payload drift must invalidate replay binding");
            round++;
                var report=Report();
                var refs=References().ToArray();
                refs[1%3]=refs[1%3] with { PayloadFingerprint="ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff" };
                Check(ProductionCaptureEvidenceReplayBindingRuntime.Validate(report,refs).Count>0 &&
                      (round==100),"payload drift must invalidate replay binding");
            round++;
                var report=Report();
                var refs=References().ToArray();
                refs[2%3]=refs[2%3] with { PayloadFingerprint="ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff" };
                Check(ProductionCaptureEvidenceReplayBindingRuntime.Validate(report,refs).Count>0 &&
                      (round==100),"payload drift must invalidate replay binding");
            round++;
                var report=Report();
                var refs=References().ToArray();
                refs[3%3]=refs[3%3] with { PayloadFingerprint="ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff" };
                Check(ProductionCaptureEvidenceReplayBindingRuntime.Validate(report,refs).Count>0 &&
                      (round==100),"payload drift must invalidate replay binding");
            round++;
                var report=Report();
                var refs=References().ToArray();
                refs[4%3]=refs[4%3] with { PayloadFingerprint="ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff" };
                Check(ProductionCaptureEvidenceReplayBindingRuntime.Validate(report,refs).Count>0 &&
                      (round==100),"payload drift must invalidate replay binding");
            round++;
                var report=Report();
                var refs=References().ToArray();
                refs[5%3]=refs[5%3] with { PayloadFingerprint="ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff" };
                Check(ProductionCaptureEvidenceReplayBindingRuntime.Validate(report,refs).Count>0 &&
                      (round==100),"payload drift must invalidate replay binding");
            round++;
                var report=Report();
                var refs=References().ToArray();
                refs[6%3]=refs[6%3] with { PayloadFingerprint="ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff" };
                Check(ProductionCaptureEvidenceReplayBindingRuntime.Validate(report,refs).Count>0 &&
                      (round==100),"payload drift must invalidate replay binding");
            round++;
                var report=Report();
                var refs=References().ToArray();
                refs[7%3]=refs[7%3] with { PayloadFingerprint="ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff" };
                Check(ProductionCaptureEvidenceReplayBindingRuntime.Validate(report,refs).Count>0 &&
                      (round==100),"payload drift must invalidate replay binding");
            round++;
                var report=Report();
                var refs=References().ToArray();
                refs[8%3]=refs[8%3] with { PayloadFingerprint="ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff" };
                Check(ProductionCaptureEvidenceReplayBindingRuntime.Validate(report,refs).Count>0 &&
                      (round==100),"payload drift must invalidate replay binding");
            round++;
                var report=Report();
                var refs=References().ToArray();
                refs[9%3]=refs[9%3] with { PayloadFingerprint="ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff" };
                Check(ProductionCaptureEvidenceReplayBindingRuntime.Validate(report,refs).Count>0 &&
                      (round==100),"payload drift must invalidate replay binding");
        }
