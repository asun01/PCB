...
public static class ProductionQualityEvidenceReplayDescriptor4HundredStageSmoke
{
...
var badCount=descriptor with {EvidenceFrameCount=9};
for(var i=0;i<10;i++) Check(!ProductionQualityEvidenceReplayDescriptorRuntime.IsValid(definition,production,qualityRun,evidence,bundle,malformed),"Malformed descriptor fingerprint should be rejected.");
for(var i=0;i<10;i++) Check(!ProductionQualityEvidenceReplayDescriptorRuntime.IsValid(definition,production,qualityRun,evidence,bundle,upper),"Uppercase descriptor fingerprint should be rejected.");
for(var i=0;i<10;i++) Check(!ProductionQualityEvidenceReplayDescriptorRuntime.IsValid(definition,production,qualityRun,evidence,bundle,badCount),"Evidence count tampering should be rejected.");
for(var i=0;i<10;i++) Check(ProductionQualityEvidenceReplayDescriptorRuntime.Validate(definition,production,qualityRun,evidence,bundle,malformed).Count>0,"Malformed fingerprint should produce diagnostics.");
for(var i=0;i<10;i++) Check(ProductionQualityEvidenceReplayDescriptorRuntime.Validate(definition,production,qualityRun,evidence,bundle,badCount).Count>0,"Count tampering should produce diagnostics.");
