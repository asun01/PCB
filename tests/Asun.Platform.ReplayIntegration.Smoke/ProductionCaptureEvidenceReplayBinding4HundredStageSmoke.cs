    ...
    public static Task RunAsync(Action<bool,string> Check)
    {
        ...
        for(var group=0;group<10;group++)
        {
            for(var i=0;i<10;i++)
            {
                ...
                Check(ProductionCaptureEvidenceReplayBindingRuntime.IsEquivalent(left,right) && left.CaptureEvidenceProjectionFingerprint==right.CaptureEvidenceProjectionFingerprint && (group<9 || round==100),"matrix");
            }
        }
