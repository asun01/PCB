...
public static class ProductionQualityEvidenceReleaseReplayBinding2HundredStageSmoke
{
    public static async ValueTask RunAsync(Action<bool,string> assert)
    {
        ...
        var badBundle=binding with {ReplayBundleFingerprint=new string('b',64)};
var badManifest=binding with {ReleaseManifestFingerprint=new string('c',64)};
for(var i=0;i<10;i++) Check(!ProductionQualityEvidenceReleaseReplayBindingRuntime.IsValid(definition,production,qualityRun,evidence,bundle,manifest,badBundle),"Bundle tampering should be rejected.");
for(var i=0;i<10;i++) Check(!ProductionQualityEvidenceReleaseReplayBindingRuntime.IsValid(definition,production,qualityRun,evidence,bundle,manifest,badManifest),"Manifest tampering should be rejected.");
for(var i=0;i<10;i++) Check(ProductionQualityEvidenceReleaseReplayBindingRuntime.Validate(definition,production,qualityRun,evidence,bundle,manifest,badBundle).Count>0,"Bundle tampering should produce diagnostics.");
for(var i=0;i<10;i++) Check(ProductionQualityEvidenceReleaseReplayBindingRuntime.Validate(definition,production,qualityRun,evidence,bundle,manifest,badManifest).Count>0,"Manifest tampering should produce diagnostics.");
...
for(var i=0;i<10;i++) Check(binding.ReleaseReady,"Baseline release readiness should remain true.");
        assert(round==100,$"ProductionQualityEvidenceReleaseReplayBinding2HundredStageSmoke should execute exactly 100 numbered rounds; actual {round}.");
    }
}
