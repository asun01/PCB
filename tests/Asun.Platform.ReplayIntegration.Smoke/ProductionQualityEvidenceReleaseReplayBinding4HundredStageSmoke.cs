...
public static class ProductionQualityEvidenceReleaseReplayBinding4HundredStageSmoke
{
...
for(var i=0;i<10;i++) Check(!ProductionQualityEvidenceReleaseReplayBindingRuntime.IsValid(definition,production,qualityRun,evidence,bundle,manifest,badReady),"Readiness tampering should be rejected.");
for(var i=0;i<10;i++) Check(!ProductionQualityEvidenceReleaseReplayBindingRuntime.IsValid(definition,production,qualityRun,evidence,bundle,manifest,malformed),"Malformed binding fingerprint should be rejected.");
for(var i=0;i<10;i++) Check(!ProductionQualityEvidenceReleaseReplayBindingRuntime.IsValid(definition,production,qualityRun,evidence,bundle,manifest,upper),"Uppercase binding fingerprint should be rejected.");
for(var i=0;i<10;i++) Check(ProductionQualityEvidenceReleaseReplayBindingRuntime.Validate(definition,production,qualityRun,evidence,bundle,manifest,badReady).Count>0,"Readiness tampering should produce diagnostics.");
for(var i=0;i<10;i++) Check(ProductionQualityEvidenceReleaseReplayBindingRuntime.Validate(definition,production,qualityRun,evidence,bundle,manifest,malformed).Count>0,"Malformed fingerprint should produce diagnostics.");
