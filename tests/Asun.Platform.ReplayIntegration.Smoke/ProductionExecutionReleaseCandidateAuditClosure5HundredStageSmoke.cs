...
public static class ProductionExecutionReleaseCandidateAuditClosure5HundredStageSmoke
{
    public static async ValueTask RunAsync(Action<bool,string> assert)
    {
        ...
        var changedManifest=ProductionReleaseCandidateRuntime.Create(
    manifest.Identity,
    definition,
    production,
    "release/changed");
var changedClosure=ProductionExecutionReleaseCandidateAuditClosureRuntime.Create(
    auditClosure,
    changedManifest,
    definition,
    production);
for(var i=0;i<10;i++) Check(changedManifest.Fingerprint!=manifest.Fingerprint,"Changed Release candidate should have a different manifest identity.");
for(var i=0;i<10;i++) Check(changedClosure.Fingerprint!=closure.Fingerprint,"Changed Release candidate should change audit identity.");
for(var i=0;i<10;i++) Check(changedClosure.ProductionSessionId==closure.ProductionSessionId,"Changed Release candidate should preserve Production session.");
for(var i=0;i<10;i++) Check(changedClosure.AuditClosureFingerprint==closure.AuditClosureFingerprint,"Changed Release candidate should preserve execution audit identity.");
for(var i=0;i<10;i++) Check(changedClosure.ArtifactPath=="release/changed","Changed Release candidate should expose changed artifact path.");
for(var i=0;i<10;i++) Check(changedClosure.ReleaseManifestFingerprint==changedManifest.Fingerprint,"Changed audit should preserve changed manifest identity.");
for(var i=0;i<10;i++) Check(ProductionExecutionReleaseCandidateAuditClosureRuntime.IsValid(auditClosure,changedManifest,definition,production,changedClosure),"Changed Release candidate audit should validate.");
for(var i=0;i<10;i++) Check(ProductionExecutionReleaseCandidateAuditClosureRuntime.Create(auditClosure,manifest,definition,production).Fingerprint==closure.Fingerprint,"Original audit creation should remain deterministic.");
for(var i=0;i<10;i++) Check(ProductionExecutionReleaseCandidateAuditClosureRuntime.IsValid(auditClosure,manifest,definition,production,closure),"Original Release candidate audit should remain valid.");
for(var i=0;i<10;i++) Check(ProductionReleaseCandidateValidationRuntime.IsValid(manifest,definition,production),"Original Release candidate should remain valid.");

        assert(round==100,$"ProductionExecutionReleaseCandidateAuditClosure5HundredStageSmoke should execute exactly 100 numbered rounds; actual {round}.");
    }
}
