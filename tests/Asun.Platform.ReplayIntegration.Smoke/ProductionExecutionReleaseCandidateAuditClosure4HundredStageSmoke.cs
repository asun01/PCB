...
public static class ProductionExecutionReleaseCandidateAuditClosure4HundredStageSmoke
{
...
for(var i=0;i<10;i++) Check(!ProductionExecutionReleaseCandidateAuditClosureRuntime.IsValid(auditClosure,badPath,definition,production,closure),"Changed artifact manifest should be rejected against original closure.");
for(var i=0;i<10;i++) Check(!ProductionExecutionReleaseCandidateAuditClosureRuntime.IsValid(auditClosure,manifest,definition,production,badReady),"Readiness tampering should be rejected.");
for(var i=0;i<10;i++) Check(ProductionExecutionReleaseCandidateAuditClosureRuntime.Validate(auditClosure,badPath,definition,production,closure).Count>0,"Changed artifact manifest should emit diagnostics.");
for(var i=0;i<10;i++) Check(ProductionExecutionReleaseCandidateAuditClosureRuntime.Validate(auditClosure,manifest,definition,production,badReady).Count>0,"Readiness tampering should emit diagnostics.");
for(var i=0;i<10;i++) Check(closure.ArtifactPath==manifest.Artifacts[0].Path,"Baseline artifact path remains stable.");
...
for(var i=0;i<10;i++) Check(closure.ReleaseReady==ReleaseReadinessRuntime.Evaluate(manifest).Ready,"Baseline readiness remains stable.");
for(var i=0;i<10;i++) Check(ProductionExecutionReleaseCandidateAuditClosureRuntime.IsValid(auditClosure,manifest,definition,production,closure),"Baseline audit closure remains valid.");

        assert(round==100,$"ProductionExecutionReleaseCandidateAuditClosure4HundredStageSmoke should execute exactly 100 numbered rounds; actual {round}.");
    }
}
