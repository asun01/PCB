...
public static class ProductionRoiMeasurementQualityContext2HundredStageSmoke
{
    public static Task RunAsync(Action<bool,string> Check)
    {
        ...
        for(var group0=0;group0<10;group0++)
        {
            round++;
                ...
                var measurement=f.Evaluation.Measurement with { ProductionInputFingerprint=new string('0',64) };
                var tampered=f.Evaluation with { Measurement=measurement };
                Check(!ProductionRoiMeasurementQualityContextRuntime.IsValid(f.Production,f.RoiContext,tampered) &&
                      (group0<9 || round==100),
                      "measurement production provenance drift must be rejected");
        }
