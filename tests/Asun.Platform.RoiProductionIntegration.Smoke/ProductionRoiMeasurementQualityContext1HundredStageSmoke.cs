...
public static class ProductionRoiMeasurementQualityContext1HundredStageSmoke
{
    public static Task RunAsync(Action<bool,string> Check)
    {
        ...
        for(var group0=0;group0<10;group0++)
        {
            round++;
                ...
                var result=ProductionRoiMeasurementQualityContextRuntime.Create(f.Production,f.RoiContext,f.Evaluation);
                Check(ProductionRoiMeasurementQualityContextRuntime.IsValid(f.Production,f.RoiContext,f.Evaluation) &&
                      result.ProductionSessionId==f.Production.SessionId &&
                      result.MeasurementSequence==1 &&
                      result.QualityResultId==f.Evaluation.Result.ResultId &&
                      result.BindingFingerprint.Length==64 &&
                      (group0<9 || round==100),
                      "clean ROI measurement quality context must be valid");
        }
