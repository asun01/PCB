...
public static class ProductionRoiQualityEvidenceReplayContext3HundredStageSmoke
{
    public static Task RunAsync(Action<bool,string> Check)
    {
        ...
        for(var group0=0;group0<10;group0++)
        {
            round++;
                ...
                var tampered=f.MeasurementQualityContext with { RoiContextBindingFingerprint=new string('d',64) };
                Check(!ProductionRoiQualityEvidenceReplayContextRuntime.IsValid(tampered,f.QualityRun,f.Bindings,f.Resolutions,f.Descriptors) &&
                      (group0<9 || round==100),
                      "ROI-to-quality context binding drift must be rejected");
        }
