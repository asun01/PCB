...
public static class CaptureSessionProductionEvidenceBinding4HundredStageSmoke
{
    public static async ValueTask RunAsync(Action<bool,string> assert)
    {
        ...
        var frames=new[]{frame1,frame2,frame3,frame4,frame5};
        ...
        for(var i=0;i<10;i++) Check(binding.FrameCount==5,"Binding should preserve Evidence frame count.");
        ...
        for(var i=0;i<10;i++) Check(provenance.Count==5,"Every Production frame should have matching provenance.");
        for(var i=0;i<10;i++) Check(evidenceReferences.Select(reference=>reference.Sequence).SequenceEqual(Enumerable.Range(1,5)),"Evidence reference sequences should remain contiguous.");
        ...
        assert(round==100,$"CaptureSessionProductionEvidenceBinding4HundredStageSmoke should execute exactly 100 numbered rounds; actual {round}.");
    }
}
