...
public static class CaptureSessionProductionEvidenceBinding5HundredStageSmoke
{
    public static async ValueTask RunAsync(Action<bool,string> assert)
    {
        ...
        var program=new InspectionProgram(
            Guid.Parse("F1000000-0000-0000-0000-000000000005"),
            "CaptureEvidenceBinding",
            new Version(1,0,0),
            new[]{
                new ProgramStep(
                    Guid.Parse("F2000000-0000-0000-0000-000000000005"),
                    1,
        ...
        var frame4=await frameSource.CaptureAsync();
        var frame5=await frameSource.CaptureAsync();
        var frame6=await frameSource.CaptureAsync();
        var frames=new[]{frame1,frame2,frame3,frame4,frame5,frame6};
        ...
        var evidenceReferences=new ProductionCaptureEvidenceFrameReference[]
        {
            ...
            new(4,frames[3].PayloadFingerprint,frames[3].Metadata.Width,frames[3].Metadata.Height,frames[3].Metadata.PixelFormat,new[]{EvidenceHandle.Create("capture/4/a")}),
            new(5,frames[4].PayloadFingerprint,frames[4].Metadata.Width,frames[4].Metadata.Height,frames[4].Metadata.PixelFormat,new[]{EvidenceHandle.Create("capture/5/a")}),
            new(6,frames[5].PayloadFingerprint,frames[5].Metadata.Width,frames[5].Metadata.Height,frames[5].Metadata.PixelFormat,new[]{EvidenceHandle.Create("capture/6/a")}),
        };
        ...
        for(var i=0;i<10;i++) Check(binding.FrameCount==6,"Binding should preserve Evidence frame count.");
        ...
        for(var i=0;i<10;i++) Check(provenance.Count==6,"Every Production frame should have matching provenance.");
        for(var i=0;i<10;i++) Check(evidenceReferences.Select(reference=>reference.Sequence).SequenceEqual(Enumerable.Range(1,6)),"Evidence reference sequences should remain contiguous.");
        ...
        assert(round==100,$"CaptureSessionProductionEvidenceBinding5HundredStageSmoke should execute exactly 100 numbered rounds; actual {round}.");
    }
}
