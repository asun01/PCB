...
public static class CaptureSessionProductionEvidenceBinding1HundredStageSmoke
{
    public static async ValueTask RunAsync(Action<bool,string> assert)
    {
        ...
        for(var i=0;i<10;i++) Check(binding.FrameCount==2,"Binding should preserve Evidence frame count.");
        ...
        for(var i=0;i<10;i++) Check(provenance.Count==2,"Every Production frame should have matching provenance.");
        for(var i=0;i<10;i++) Check(evidenceReferences.Select(reference=>reference.Sequence).SequenceEqual(Enumerable.Range(1,2)),"Evidence reference sequences should remain contiguous.");
        ...
        assert(round==100,$"CaptureSessionProductionEvidenceBinding1HundredStageSmoke should execute exactly 100 numbered rounds; actual {round}.");
    }
}
