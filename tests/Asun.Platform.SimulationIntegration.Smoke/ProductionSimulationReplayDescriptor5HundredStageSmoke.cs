...
public static class ProductionSimulationReplayDescriptor5HundredStageSmoke
{
    public static async ValueTask RunAsync(Action<bool,string> assert)
    {
        ...
        var reversed=observations.Reverse().ToArray();
        var equivalent=ProductionSimulationReplayBindingRuntime.Create(production,reversed);
        var equivalentDescriptor=ProductionSimulationReplayDescriptorRuntime.Create(production,reversed,equivalent);
        for(var i=0;i<10;i++) Check(equivalentDescriptor.ProductionSessionId==descriptor.ProductionSessionId,"Reordered simulation observations should preserve session identity.");
        for(var i=0;i<10;i++) Check(equivalentDescriptor.ProductionFingerprint==descriptor.ProductionFingerprint,"Reordered simulation observations should preserve production identity.");
        for(var i=0;i<10;i++) Check(equivalentDescriptor.FrameCount==descriptor.FrameCount,"Reordered simulation observations should preserve frame count.");
        for(var i=0;i<10;i++) Check(equivalentDescriptor.BindingFingerprint==descriptor.BindingFingerprint,"Binding should canonicalize simulation observation order.");
        for(var i=0;i<10;i++) Check(equivalentDescriptor.DescriptorFingerprint==descriptor.DescriptorFingerprint,"Descriptor should canonicalize simulation observation order.");
        for(var i=0;i<10;i++) Check(ProductionSimulationReplayDescriptorRuntime.IsEquivalent(descriptor,equivalentDescriptor),"Equivalent reordered descriptors should compare equal.");
