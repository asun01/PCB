# HDI FPC RigidFlex Workbench
- 文档 ID：`UI-PCS-016`
- 版本：`2.0.0`
- 状态：`ImplementationSpecificationReady`
- 技术路线：WPF + DevExpress
- Scope：layer stack, via, fine pattern, R2R/non-rigid and composite workflows
...
**页面角色：** `HDI/FPC/RigidFlex Workbench`

### 关键 Command

`SelectLayer` · `SelectLocalFrame` · `RunLocalInspection` · `OpenTransform` · `OpenEvidence` · `ReviewException`

上述 Command 只定义 UI 意图；实际权限、Owner 和副作用必须由 Application Port/State-Owner Contract 决定。

### 关键 Query / 对象

`LayerStackRevision` · `WebCoordinateReference` · `NonRigidTransform` · `InspectionTarget`

### 页面硬边界

['Local transforms require residual/validity gates']
