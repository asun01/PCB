...
SPC。输入：`MetricDefinition/SPC projection`；输出/投影：`控制图、分母、水位、迟到数据`。页面不是事实权威。

## 2. 工作台布局

`Grid/Chart + denominator inspector + trace back`
...
`SelectMetric` · `SelectDenominator` · `FilterWindow` · `ShowControlChart` · `InvestigatePoint` · `OpenEvidence` · `CreateAction`

上述 Command 只定义 UI 意图；实际权限、Owner 和副作用必须由 Application Port/State-Owner Contract 决定。

### 关键 Query / 对象

`MetricDefinition` · `SPCDataset` · `SubgroupPolicy` · `Watermark`

### 页面硬边界

['Chart projection cannot change source facts or denominator']
