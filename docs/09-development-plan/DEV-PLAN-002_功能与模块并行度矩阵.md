...
- 适用范围：计划排期和研发资源分配
- 直接依赖：`DEV-PLAN-001`, `DEV-GOV-005`
- 主要输出：模块之间的串并行关系和资源冲突矩阵
...
| 能力 | 等级 | 前置 | 可与谁并行 |
|---|---:|---|---|
| Contract/Schema/Validator | P0 | 架构 | 术语、研究 |
| UI Shell/Design System | P1 | PageContract | Vision/Runtime |
| Vision Contract | P1 | Contract | UI/Identity |
| Identity/BoardRun | P1 | Contract | Vision/UI |
| Evidence/Commit | P1 | Contract | UI shell |
| Calibration/Coordinate | P1 | Contract | UI/Runtime |
| HALCON Benchmarks | P1 | Vision Contract | UI/Domain |
| Device adapters | HIL/P1 | Device spec | Runtime/Tests |
| SPI domain | P2 | Core minimal chain | PCB/AOI domain prep |
| PCB domain | P2 | Core minimal chain | Stencil/HDI/FPC prep |
| AI | P2/D | Replay data | UI improvements/Quality |
| Production Acceptance | QUAL | all relevant | 不得与未通过的前置串行冲突 |
