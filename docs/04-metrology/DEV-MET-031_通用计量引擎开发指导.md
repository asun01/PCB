...
- 适用范围：测量对象、关系测量、采样、拟合、异常点、不确定度
- 直接依赖：`DEV-VIS-025`, `DEV-GOV-004`
- 主要输出：MeasurementSpec → MeasurementPlan → FeatureMeasurement → MeasurementSet
...
本指导的具体功能以以下实施规范为准：FS-017。每个 FS-xxx 都包含输入/输出、算法、鲁棒性、性能、UI、测试、验收和禁止实现。
