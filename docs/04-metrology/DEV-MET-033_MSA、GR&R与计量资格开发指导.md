...
- 主要输出：MetrologyReferenceCertificate、QualificationContext
...
资格证书至少绑定：标准件/参考件、机台、镜头/传感器、CalibrationProfile、算法版本、单位、环境条件、试验日期、统计方法、结果及有效期。资格失效时生产 CapabilityResolver 必须拒绝把该能力当成已批准计量能力。


## 实施级关联功能

本指导的具体功能以以下实施规范为准：FS-018。每个 FS-xxx 都包含输入/输出、算法、鲁棒性、性能、UI、测试、验收和禁止实现。
