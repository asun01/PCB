...
将 MetricDefinition 与 subgroup/denominator 规则投影成质量统计。
...
1. 读取 committed facts/approved inputs。
2. 固定分母、过滤规则、水位。
3. 生成 chart data。
4. 迟到数据独立标识。
5. 历史结果可重算但不改事实。
...
**Primary route：** `MetricDefinition + subgroup rules`

**Fallback route：** `No semantic fallback; render “not computable” on data gaps`

**Fallback trigger：** A chart may degrade visually but not change denominator or quality interpretation.

**Reject / Block conditions：** Missing MetricDefinition, subgroup mismatch, watermark gap, denominator conflict.

**参数键（名称冻结，数值由批准 Profile / Recipe / Qualification 提供）：** `['MetricDefinitionId', 'SubgroupPolicyId', 'DenominatorPolicyId', 'WatermarkId']`

**结果语义：** `Chart projection + gap markers`
...
| 变化/故障源 | 必测处理 | 结果要求 |
|---|---|---|
| 分母变化 | 注入/回放 分母变化 | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
| 跨工序混算 | 注入/回放 跨工序混算 | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
...
| PerformanceProfile | `PF-FS-064` | All numeric targets must come from the approved profile; missing profile values are Unverified/Blocker. |
...
|---|---|---|
| Primary | `MetricDefinition + subgroup rules` | 仅在本 FS 的适用域、输入质量门和参数来源满足时执行。 |
| Fallback | `No semantic fallback; render “not computable” on data gaps` | A chart may degrade visually but not change denominator or quality interpretation. |
| Reject / Block | `Missing MetricDefinition, subgroup mismatch, watermark gap, denominator conflict.` | 保留证据与原因；不得生成权威 OK。 |
...
|---|---|---|
| Port/interface | `ISPCChart` | Stable application/domain-facing contract; no vendor type leakage |
| Implementation | `SPCChart.cs` | Pipeline/algorithm implementation |
| Validator | `SPCChartValidator.cs` | Input/precondition/quality checks |
| Tests | `SPCChartTests.cs` | unit/contract/integration/fault tests |
| Golden/Replay | `tests/Golden/FS-064/` | fixed datasets, expected outputs, provenance |
| Benchmark | `SPCChartBenchmark.cs` | P50/P95/P99/resource evidence |
...
|---|---|---|
| T64-01 | contract/schema validation | expected assertion + evidence + trace; no silent pass |
| T64-02 | normal + boundary + reject | expected assertion + evidence + trace; no silent pass |
| T64-03 | replay/golden dataset | expected assertion + evidence + trace; no silent pass |
| T64-04 | fault injection | expected assertion + evidence + trace; no silent pass |
| T64-05 | performance benchmark | expected assertion + evidence + trace; no silent pass |
...
| 项目 | 冻结值 | 规则 |
|---|---|---|
| Function | `FS-064` | 本任务只实现该 FS；不得顺手扩展其它 FS。 |
| Guide | `DEV-DOM-058A` | 指导文档是实施语义权威。 |
| Project | `Asun.Domain.Quality` | 首选代码项目；跨项目修改必须有依赖记录。 |
| Port | `ISPCChart` | 公共入口名称固定；改变名称必须先改 Contract/Registry。 |
| PageContract | `UI-PCS-008` | UI 只做投影/Command client；不得成为事实权威。 |
| Algorithm baseline | `MetricDefinition + subgroup rules` | 只能在明确的条件下切换到 Fallback；不得凭个人偏好替换。 |
...
| Primary algorithm/route | `MetricDefinition + subgroup rules` |
| Fallback | `No semantic fallback; render “not computable” on data gaps` |
| Fallback trigger | A chart may degrade visually but not change denominator or quality interpretation. |
| Reject/Block | Missing MetricDefinition, subgroup mismatch, watermark gap, denominator conflict. |
| Parameter authority | `['MetricDefinitionId', 'SubgroupPolicyId', 'DenominatorPolicyId', 'WatermarkId']` come from versioned Contract/Recipe/MethodPolicy/Qualification; no code constants |
| Result/evidence | Chart projection + gap markers + input snapshot + algorithm/parameter version + quality-gate outcome |
