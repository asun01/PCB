...
2. 只对有效像素积分。
3. 面积/像素尺度必须来源于 calibration。
4. 体积 = Σ(height × valid pixel area)，插值区域必须带标记。
...
**Reject / Block conditions：** Invalid reference, insufficient valid surface, negative/NaN integral, coverage below floor.
...
| Reject / Block | `Invalid reference, insufficient valid surface, negative/NaN integral, coverage below floor.` | 保留证据与原因；不得生成权威 OK。 |
...
| Fallback trigger | Do not substitute a different measurand or fill invalid surface points without approved imputation policy. |
| Reject/Block | Invalid reference, insufficient valid surface, negative/NaN integral, coverage below floor. |
