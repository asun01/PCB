...
将多个 3D 模型注册到统一坐标，输出变换、score、残差及图一致性。
...
1. 校验 rough transforms。
2. 对 pair/全局图求解相对变换；`register_object_model_3d_global` 适用于基于重叠优化的全局细化。
3. 检查 score、残差、连接图和异常节点。
4. 低 overlap 或离群节点不强行吸收进全局解。
5. 生成可回放 transform artifact。
...
**参数键（名称冻结，数值由批准 Profile / Recipe / Qualification 提供）：** `['MinOverlap', 'MaxRegistrationResidual', 'InitialTransformSource', 'MaxIterations']`

**结果语义：** `Transform + score/residual`
