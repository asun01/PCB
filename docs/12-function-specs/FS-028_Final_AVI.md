...
3. 精检与上下文特征。
4. 去重/合并候选。
5. Review routing + evidence。

## 5. 算法/规则设计

**实施分类：** `PCB Inspection`

**Primary route：** `candidate detection + review routing`

**Fallback route：** `Conservative Review route`

**Fallback trigger：** Any ambiguous candidate is routed to Review; uncertainty cannot be turned into a clean FinalAVI result.
...
**结果语义：** `InspectionFact/ReviewClaim`
...
| Algorithm baseline | `candidate detection + review routing` |
...
|---|---|---|
| Primary | `candidate detection + review routing` | 仅在本 FS 的适用域、输入质量门和参数来源满足时执行。 |
| Fallback | `Conservative Review route` | Any ambiguous candidate is routed to Review; uncertainty cannot be turned into a clean FinalAVI result. |
...
| Algorithm baseline | `candidate detection + review routing` | 只能在明确的条件下切换到 Fallback；不得凭个人偏好替换。 |
...
| Primary algorithm/route | `candidate detection + review routing` |
| Fallback | `Conservative Review route` |
| Fallback trigger | Any ambiguous candidate is routed to Review; uncertainty cannot be turned into a clean FinalAVI result. |
...
| Result/evidence | InspectionFact/ReviewClaim + input snapshot + algorithm/parameter version + quality-gate outcome |
