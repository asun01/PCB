...
1. 计算计划 coverage。
2. 记录实际执行/跳过/拒识/缺失。
3. 形成实际 CoverageResult，并保留 planned vs actual。
4. 覆盖不足只能形成质量状态/风险，不能自动补偿为 OK。
5. 统计域必须固定分母和水位。
...
**Fallback trigger：** Missing/unknown coverage is a coverage defect, not a reason to pass or widen criteria.

**Reject / Block conditions：** Required target absent, unknown applicability, stale TargetSet, invalid sampling policy.
...
| 变化/故障源 | 必测处理 | 结果要求 |
|---|---|---|
| unknown target | 注入/回放 unknown target | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
| missing actual evidence | 注入/回放 missing actual evidence | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
| partial execution | 注入/回放 partial execution | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
...
| Fallback | `No semantic fallback; recompute from frozen CoveragePolicy` | Missing/unknown coverage is a coverage defect, not a reason to pass or widen criteria. |
| Reject / Block | `Required target absent, unknown applicability, stale TargetSet, invalid sampling policy.` | 保留证据与原因；不得生成权威 OK。 |
...
|---|---|---|
| T19-01 | contract/schema validation | expected assertion + evidence + trace; no silent pass |
| T19-02 | normal + boundary + reject | expected assertion + evidence + trace; no silent pass |
| T19-03 | replay/golden dataset | expected assertion + evidence + trace; no silent pass |
| T19-04 | fault injection | expected assertion + evidence + trace; no silent pass |
| T19-05 | performance benchmark | expected assertion + evidence + trace; no silent pass |
| T19-06 | reference standard | expected assertion + evidence + trace; no silent pass |
| T19-07 | repeatability/reproducibility | expected assertion + evidence + trace; no silent pass |
| T19-08 | uncertainty contribution | expected assertion + evidence + trace; no silent pass |
...
| Fallback trigger | Missing/unknown coverage is a coverage defect, not a reason to pass or widen criteria. |
| Reject/Block | Required target absent, unknown applicability, stale TargetSet, invalid sampling policy. |
