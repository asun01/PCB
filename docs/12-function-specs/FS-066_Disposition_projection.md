...
从已提交事实与 QualityPlan 投影处置建议/状态，不重新判定事实。
...
1. 读取 committed facts/watermarks。
2. 解析 required evidence。
3. 根据质量计划生成 projection。
4. 缺失/迟到显式显示。
5. 写入 review/authorization only through owner。
...
**Reject / Block conditions：** Missing committed fact, stale watermark, conflicting quality plan, uncommitted candidate.

**参数键（名称冻结，数值由批准 Profile / Recipe / Qualification 提供）：** `['QualityPlanRevisionId', 'FactWatermarkId', 'StalenessPolicyId']`
...
| 变化/故障源 | 必测处理 | 结果要求 |
|---|---|---|
| 迟到 | 注入/回放 迟到 | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
| 证据缺口 | 注入/回放 证据缺口 | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
| 权限限制 | 注入/回放 权限限制 | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
...
| Reject / Block | `Missing committed fact, stale watermark, conflicting quality plan, uncommitted candidate.` | 保留证据与原因；不得生成权威 OK。 |
...
committed facts or approved inputs
  ...
  → Disposition projection computation
  → quality gate
...
| Implementation | `DispositionProjection.cs` | Pipeline/algorithm implementation |
...
|---|---|---|
| T66-01 | contract/schema validation | expected assertion + evidence + trace; no silent pass |
| T66-02 | normal + boundary + reject | expected assertion + evidence + trace; no silent pass |
| T66-03 | replay/golden dataset | expected assertion + evidence + trace; no silent pass |
| T66-04 | fault injection | expected assertion + evidence + trace; no silent pass |
| T66-05 | performance benchmark | expected assertion + evidence + trace; no silent pass |
...
| Fallback trigger | When required committed facts are stale/missing, project a gated/incomplete state and expose the gap. |
| Reject/Block | Missing committed fact, stale watermark, conflicting quality plan, uncommitted candidate. |
| Parameter authority | `['QualityPlanRevisionId', 'FactWatermarkId', 'StalenessPolicyId']` come from versioned Contract/Recipe/MethodPolicy/Qualification; no code constants |
| Result/evidence | Disposition projection + gap evidence + input snapshot + algorithm/parameter version + quality-gate outcome |
