...
1. 读取 committed facts。
2. 解析 subgroup/denominator/watermark。
3. 计算统计量和控制图点。
4. 标识迟到/缺口。
5. 不回写历史事实。
...
**Reject / Block conditions：** Unknown denominator, stale watermark, subgroup invalid, insufficient samples.

**参数键（名称冻结，数值由批准 Profile / Recipe / Qualification 提供）：** `['MetricDefinitionId', 'SubgroupPolicyId', 'WatermarkId', 'MinimumSampleRule']`
...
| Reject / Block | `Unknown denominator, stale watermark, subgroup invalid, insufficient samples.` | 保留证据与原因；不得生成权威 OK。 |
...
| Fallback trigger | No backfill or denominator change merely to produce a chart. |
| Reject/Block | Unknown denominator, stale watermark, subgroup invalid, insufficient samples. |
