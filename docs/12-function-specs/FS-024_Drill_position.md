...
1. 建立板级坐标/层坐标。
2. 提取孔中心。
3. 通过 CoordinateGraph 转换。
4. 与 nominal datum 配对并计算 ΔX/ΔY/径向误差。
5. 输出 quality 与 evidence。
...
|---|---|---|
| T24-01 | contract/schema validation | expected assertion + evidence + trace; no silent pass |
| T24-02 | normal + boundary + reject | expected assertion + evidence + trace; no silent pass |
| T24-03 | replay/golden dataset | expected assertion + evidence + trace; no silent pass |
| T24-04 | fault injection | expected assertion + evidence + trace; no silent pass |
| T24-05 | performance benchmark | expected assertion + evidence + trace; no silent pass |
| T24-06 | real domain consumer | expected assertion + evidence + trace; no silent pass |
| T24-07 | hard-negative/false-positive set | expected assertion + evidence + trace; no silent pass |
