...
输入契约：`Image + model`；输出契约：`pose/score`。若字段尚未在现有 Schema 中冻结，必须登记到对应 ContractCandidate（通常 C-01/C-02/C-03/C-04/C-08），不能在功能目录中另建 JSON/数据库模型。
...
1. 先进行低成本 ROI 限制与候选预筛。
2. 主路线使用 `find_shape_model` 或经 Benchmark 选定的同类模型匹配。
3. 执行候选去重、姿态范围检查和质量门。
4. 低 score/多峰/遮挡过大进入 fallback 或 reject，而不是强行取第一名。
...
**Fallback route：** `Approved edge/contour registration route from DEV-VIS-021`

**Fallback trigger：** Trigger only when match quality fails but target observability and the approved fallback domain remain valid.
...
**参数键（名称冻结，数值由批准 Profile / Recipe / Qualification 提供）：** `['MinAcceptScore', 'MaxPoseResidual', 'MaxCandidateCount', 'FallbackEnabled']`
...
| Fallback | `Approved edge/contour registration route from DEV-VIS-021` | Trigger only when match quality fails but target observability and the approved fallback domain remain valid. |
...
| Guide | `DEV-VIS-021` | 指导文档是实施语义权威。 |
| Project | `Asun.Vision.Halcon` | 首选代码项目；跨项目修改必须有依赖记录。 |
| Port | `IShapeLocalizer` | 公共入口名称固定；改变名称必须先改 Contract/Registry。 |
...
| Fallback | `Approved edge/contour registration route from DEV-VIS-021` |
| Fallback trigger | Trigger only when match quality fails but target observability and the approved fallback domain remain valid. |
