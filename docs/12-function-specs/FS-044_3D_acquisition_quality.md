...
1. 检查 sensor state/calibration。
2. 构建 HeightField/ValidMask。
3. 计算有效率、饱和、遮挡等质量指标。
4. 建立 quality gate。
5. 只有满足 gate 才进入 paste metrology。

## 5. 算法/规则设计

**实施分类：** `SPI 3D Quality`

**Primary route：** `sensor validity + ValidMask`

**Fallback route：** `Bounded re-acquisition`

**Fallback trigger：** Retry acquisition only within the frozen retry budget and same sensor configuration; repeated invalidity becomes Blocked.
...
**结果语义：** `AcquisitionQuality + ValidMask`
...
| Algorithm baseline | `sensor validity + ValidMask` |
...
|---|---|---|
| Primary | `sensor validity + ValidMask` | 仅在本 FS 的适用域、输入质量门和参数来源满足时执行。 |
| Fallback | `Bounded re-acquisition` | Retry acquisition only within the frozen retry budget and same sensor configuration; repeated invalidity becomes Blocked. |
...
| Algorithm baseline | `sensor validity + ValidMask` | 只能在明确的条件下切换到 Fallback；不得凭个人偏好替换。 |
...
| Primary algorithm/route | `sensor validity + ValidMask` |
