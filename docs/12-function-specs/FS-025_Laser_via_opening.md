...
1. 绑定 ViaDefinition/LaserVia target。
2. 局部增强并提取 opening contour。
3. 执行几何拟合/尺寸计算。
4. 校验 shape quality 和 layer scope。
5. 按 HDI/PCB 规格判定。
...
|---|---|---|
| T25-01 | contract/schema validation | expected assertion + evidence + trace; no silent pass |
| T25-02 | normal + boundary + reject | expected assertion + evidence + trace; no silent pass |
| T25-03 | replay/golden dataset | expected assertion + evidence + trace; no silent pass |
| T25-04 | fault injection | expected assertion + evidence + trace; no silent pass |
| T25-05 | performance benchmark | expected assertion + evidence + trace; no silent pass |
| T25-06 | real domain consumer | expected assertion + evidence + trace; no silent pass |
| T25-07 | hard-negative/false-positive set | expected assertion + evidence + trace; no silent pass |
