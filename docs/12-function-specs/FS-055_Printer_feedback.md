...
输入契约：`3D/PAD/stencil/process evidence`；输出契约：`MeasurementSet/Decision/SPC/feedback`。若字段尚未在现有 Schema 中冻结，必须登记到对应 ContractCandidate（通常 C-01/C-02/C-03/C-04/C-08），不能在功能目录中另建 JSON/数据库模型。
...
1. 从 committed SPI evidence 生成 proposal。
2. 通过 capability/limit/policy 校验。
3. Approval 后发送 command。
4. 记录 FeedbackReceipt。
5. 以 EffectObservation 验证效果；不得自动修改检测阈值。
...
**Fallback route：** `Hold as Proposal until Applied/ACK evidence`

**Fallback trigger：** If ACK is absent or contradicts requested scope, do not represent command as applied.

**Reject / Block conditions：** Scope mismatch, version mismatch, no ACK, negative ACK, unsafe/unqualified change.

**参数键（名称冻结，数值由批准 Profile / Recipe / Qualification 提供）：** `['FeedbackScope', 'ParameterScope', 'CommandId', 'AppliedAckPolicy']`
...
| Output | `MeasurementSet/Decision/SPC/feedback` |
...
| Fallback | `Hold as Proposal until Applied/ACK evidence` | If ACK is absent or contradicts requested scope, do not represent command as applied. |
| Reject / Block | `Scope mismatch, version mismatch, no ACK, negative ACK, unsafe/unqualified change.` | 保留证据与原因；不得生成权威 OK。 |
...
- 输出契约：`MeasurementSet/Decision/SPC/feedback`
...
| Fallback | `Hold as Proposal until Applied/ACK evidence` |
| Fallback trigger | If ACK is absent or contradicts requested scope, do not represent command as applied. |
| Reject/Block | Scope mismatch, version mismatch, no ACK, negative ACK, unsafe/unqualified change. |
| Parameter authority | `['FeedbackScope', 'ParameterScope', 'CommandId', 'AppliedAckPolicy']` come from versioned Contract/Recipe/MethodPolicy/Qualification; no code constants |
