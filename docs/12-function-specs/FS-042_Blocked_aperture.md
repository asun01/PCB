...
1. 目标 opening 关联。
2. 比较空孔 reference / surface evidence。
3. 候选区域生成。
4. 结合高度/面积/连通性。
5. 进入 Stencil rule/review。
...
**Reject / Block conditions：** Blocked candidate below review gate, missing aperture reference, invalid height mask.
...
| Reject / Block | `Blocked candidate below review gate, missing aperture reference, invalid height mask.` | 保留证据与原因；不得生成权威 OK。 |
...
| Fallback trigger | Ambiguity routes to Review; no automatic clean result. |
| Reject/Block | Blocked candidate below review gate, missing aperture reference, invalid height mask. |
