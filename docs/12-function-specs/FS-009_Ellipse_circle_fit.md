...
1. 选择闭合或有效弧段。
2. 采用 `fit_ellipse_contour_xld` 或经 Benchmark 选定的鲁棒拟合。
3. 输出中心、轴向、半径、残差和有效弧范围。
4. 对离群点进行显式处理；不能静默删除真实坏点。
5. 对 circle 特例检查椭圆轴一致性和适用域。

## 5. 算法/规则设计

**实施分类：** `Vision/Metrology`

**Primary route：** `fit_ellipse_contour_xld or alternative fit`

**Fallback route：** `Geometry-class-specific fit: circle or ellipse`
...
**参数键（名称冻结，数值由批准 Profile / Recipe / Qualification 提供）：** `['FitGeometryClass', 'MinArcCoverage', 'MaxFitResidual', 'OutlierPolicy']`
...
| Algorithm baseline | `fit_ellipse_contour_xld or alternative fit` |
...
|---|---|---|
| Primary | `fit_ellipse_contour_xld or alternative fit` | 仅在本 FS 的适用域、输入质量门和参数来源满足时执行。 |
| Fallback | `Geometry-class-specific fit: circle or ellipse` | Select the fit only from the frozen target geometry class; a failed fit cannot silently change the feature type. |
...
| Algorithm baseline | `fit_ellipse_contour_xld or alternative fit` | 只能在明确的条件下切换到 Fallback；不得凭个人偏好替换。 |
...
| Primary algorithm/route | `fit_ellipse_contour_xld or alternative fit` |
| Fallback | `Geometry-class-specific fit: circle or ellipse` |
| Fallback trigger | Select the fit only from the frozen target geometry class; a failed fit cannot silently change the feature type. |
