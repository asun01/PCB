...
1. 解析输入模式与缺失处理。
2. 生成 component/lead/pad target。
3. 套用库中已批准 inspection templates。
4. 产生 unresolved/missing list。
5. 输出 Draft，不直接 Release。
...
**Reject / Block conditions：** Ambiguous mapping, missing source, incompatible board revision, target outside coverage.
...
| 变化/故障源 | 必测处理 | 结果要求 |
|---|---|---|
| 缺 BOM | 注入/回放 缺 BOM | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
| 库缺项 | 注入/回放 库缺项 | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
| 坐标系冲突 | 注入/回放 坐标系冲突 | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
| 料号替换歧义 | 注入/回放 料号替换歧义 | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
...
| Reject / Block | `Ambiguous mapping, missing source, incompatible board revision, target outside coverage.` | 保留证据与原因；不得生成权威 OK。 |
...
|---|---|---|
| T56-01 | contract/schema validation | expected assertion + evidence + trace; no silent pass |
| T56-02 | normal + boundary + reject | expected assertion + evidence + trace; no silent pass |
| T56-03 | replay/golden dataset | expected assertion + evidence + trace; no silent pass |
| T56-04 | fault injection | expected assertion + evidence + trace; no silent pass |
| T56-05 | performance benchmark | expected assertion + evidence + trace; no silent pass |
| T56-06 | real domain consumer | expected assertion + evidence + trace; no silent pass |
| T56-07 | hard-negative/false-positive set | expected assertion + evidence + trace; no silent pass |
...
| Fallback trigger | Fallback to another source only when it is declared Present/Valid and the mapping policy allows it; never silently substitute. |
| Reject/Block | Ambiguous mapping, missing source, incompatible board revision, target outside coverage. |
