...
4. 写 Outbox/receipt，并进行 read-back verification。
...
| 变化/故障源 | 必测处理 | 结果要求 |
|---|---|---|
| partial upload | 注入/回放 partial upload | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
| hash mismatch | 注入/回放 hash mismatch | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
| duplicate commit | 注入/回放 duplicate commit | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
| network loss | 注入/回放 network loss | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
| outbox replay | 注入/回放 outbox replay | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
...
UI/工作区：Evidence viewer。交互遵循“状态→对象→结果→下一动作”；任何生产配置修改必须 Draft→Validate→Approve→Release；异常必须一键定位到证据。
...
| Validator | `EvidenceCommitServiceValidator.cs` | Input/precondition/quality checks |
