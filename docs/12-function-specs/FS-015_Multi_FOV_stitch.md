...
将多 FOV 图像/几何证据融合到全局坐标，控制累计漂移并支持大图交互。
...
1. 建立 position graph。
2. 优先用标定/平台坐标提供初值，再用 overlap evidence 校正。
3. 全局优化时监控残差与闭环误差。
4. 生成 seam/coverage/invalid regions。
5. 渲染采用 Tile/Pyramid/LOD，禁止整板每次重新解码。
...
**Fallback trigger：** A failed FOV may be excluded only when CoveragePolicy marks it optional and the resulting coverage remains acceptable; otherwise reject the stitch.

**Reject / Block conditions：** Graph underconstrained, overlap inconsistency, residual above gate, required-FOV missing.

**参数键（名称冻结，数值由批准 Profile / Recipe / Qualification 提供）：** `['MaxSeamResidual', 'MaxTransformResidual', 'MinOverlap', 'RequiredFovPolicy']`
...
| Fallback | `Local graph re-solve / qualified FOV exclusion` | A failed FOV may be excluded only when CoveragePolicy marks it optional and the resulting coverage remains acceptable; otherwise reject the stitch. |
| Reject / Block | `Graph underconstrained, overlap inconsistency, residual above gate, required-FOV missing.` | 保留证据与原因；不得生成权威 OK。 |
...
| Fallback trigger | A failed FOV may be excluded only when CoveragePolicy marks it optional and the resulting coverage remains acceptable; otherwise reject the stitch. |
| Reject/Block | Graph underconstrained, overlap inconsistency, residual above gate, required-FOV missing. |
...
| Result/evidence | Global coordinate map + seam/residual report + input snapshot + algorithm/parameter version + quality-gate outcome |
