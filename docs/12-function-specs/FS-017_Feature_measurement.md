...
把几何特征转成可解释 FeatureMeasurement/MeasurementSet，并区分 Raw/Canonical/Display。

**不负责：** 生产机判、最终处置、真实设备安全功能。

## 2. 权威与契约

输入契约：`feature geometry`；输出契约：`FeatureMeasurement`。若字段尚未在现有 Schema 中冻结，必须登记到对应 ContractCandidate（通常 C-01/C-02/C-03/C-04/C-08），不能在功能目录中另建 JSON/数据库模型。
...
坐标系、单位、FeatureDefinition、采样策略、资格。
...
1. 提取基础 feature。
2. 在 reference frame 中规范化单位。
3. 按 RelationshipType 计算单特征或特征关系，明确 OperandA/B、PairingRule、Direction。
4. 产生 MeasurementQuality 和 provenance。
5. DisplayValue 仅为投影，不参与生产判定。

## 5. 算法/规则设计

**实施分类：** `Metrology`

**Primary route：** `edge/fit/distance/relationship`

**Fallback route：** `Approved alternate measurement method in same measurement family`

**Fallback trigger：** Fallback may change numerical method only when the MethodPolicy/Qualification already declares equivalence for the same measurand.

**Reject / Block conditions：** Invalid reference, insufficient samples, method applicability failure, uncertainty budget failure.

**参数键（名称冻结，数值由批准 Profile / Recipe / Qualification 提供）：** `['MeasurementMethodId', 'MinSampleCount', 'MaxResidual', 'UncertaintyBudgetId']`

**结果语义：** `FeatureMeasurement + quality/uncertainty`
...
| 变化/故障源 | 必测处理 | 结果要求 |
|---|---|---|
| missing operand | 注入/回放 missing operand | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
| invalid quality | 注入/回放 invalid quality | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
| domain mismatch | 注入/回放 domain mismatch | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
...
UI/工作区：metrology workspace。交互遵循“状态→对象→结果→下一动作”；任何生产配置修改必须 Draft→Validate→Approve→Release；异常必须一键定位到证据。

Measurement workspace 显示 Nominal/Raw/Canonical/Display 四层值、参考坐标、OperandA/B 和质量原因。
...
| PerformanceProfile | `PF-FS-017` | All numeric targets must come from the approved profile; missing profile values are Unverified/Blocker. |
...
| Algorithm baseline | `edge/fit/distance/relationship` |
...
|---|---|---|
| Primary | `edge/fit/distance/relationship` | 仅在本 FS 的适用域、输入质量门和参数来源满足时执行。 |
| Fallback | `Approved alternate measurement method in same measurement family` | Fallback may change numerical method only when the MethodPolicy/Qualification already declares equivalence for the same measurand. |
| Reject / Block | `Invalid reference, insufficient samples, method applicability failure, uncertainty budget failure.` | 保留证据与原因；不得生成权威 OK。 |
...
| Performance budget | `P95 per measurement` | 由 PerformanceProfile 绑定；至少记录 P50/P95/P99、峰值内存、并发安全和取消延迟；未实测不得声称 Qualified |
...
feature geometry
  ...
  → Feature measurement computation
  → quality gate
...
|---|---|---|
| Port/interface | `IFeatureMeasurementService` | Stable application/domain-facing contract; no vendor type leakage |
| Implementation | `FeatureMeasurementService.cs` | Pipeline/algorithm implementation |
| Validator | `FeatureMeasurementServiceValidator.cs` | Input/precondition/quality checks |
| Tests | `FeatureMeasurementServiceTests.cs` | unit/contract/integration/fault tests |
| Golden/Replay | `tests/Golden/FS-017/` | fixed datasets, expected outputs, provenance |
| Benchmark | `FeatureMeasurementServiceBenchmark.cs` | P50/P95/P99/resource evidence |
...
|---|---|---|
| T17-01 | contract/schema validation | expected assertion + evidence + trace; no silent pass |
| T17-02 | normal + boundary + reject | expected assertion + evidence + trace; no silent pass |
| T17-03 | replay/golden dataset | expected assertion + evidence + trace; no silent pass |
| T17-04 | fault injection | expected assertion + evidence + trace; no silent pass |
| T17-05 | performance benchmark | expected assertion + evidence + trace; no silent pass |
| T17-06 | reference standard | expected assertion + evidence + trace; no silent pass |
| T17-07 | repeatability/reproducibility | expected assertion + evidence + trace; no silent pass |
| T17-08 | uncertainty contribution | expected assertion + evidence + trace; no silent pass |
...
| 项目 | 冻结值 | 规则 |
|---|---|---|
| Function | `FS-017` | 本任务只实现该 FS；不得顺手扩展其它 FS。 |
| Guide | `DEV-MET-031` | 指导文档是实施语义权威。 |
| Project | `Asun.Metrology.Core` | 首选代码项目；跨项目修改必须有依赖记录。 |
| Port | `IFeatureMeasurementService` | 公共入口名称固定；改变名称必须先改 Contract/Registry。 |
| PageContract | `UI-PCS-006` | UI 只做投影/Command client；不得成为事实权威。 |
| Algorithm baseline | `edge/fit/distance/relationship` | 只能在明确的条件下切换到 Fallback；不得凭个人偏好替换。 |
| SideEffectPolicy | `none; 无副作用；本功能不得直接写入生产事实、设备状态、放行状态或外部控制副作用。` | 冻结策略；任何未批准副作用路径必须 Blocker。 |
| Performance focus | `P95 per measurement` | 未测量前不得声称满足性能。 |
...
- 输入契约：`feature geometry`
- 输出契约：`FeatureMeasurement`
...
|---|---|
| Function class | `Metrology` |
| Primary algorithm/route | `edge/fit/distance/relationship` |
| Fallback | `Approved alternate measurement method in same measurement family` |
| Fallback trigger | Fallback may change numerical method only when the MethodPolicy/Qualification already declares equivalence for the same measurand. |
| Reject/Block | Invalid reference, insufficient samples, method applicability failure, uncertainty budget failure. |
| Parameter authority | `['MeasurementMethodId', 'MinSampleCount', 'MaxResidual', 'UncertaintyBudgetId']` come from versioned Contract/Recipe/MethodPolicy/Qualification; no code constants |
| Result/evidence | FeatureMeasurement + quality/uncertainty + input snapshot + algorithm/parameter version + quality-gate outcome |
| UI rule | PageContract may project and command; it must not create/modify the underlying fact directly. |
| Performance | `P95 per measurement`; target values come only from the applicable PerformanceProfile. |
