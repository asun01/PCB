...
1. 在 valid mask 内计算高度分布。
2. 至少保存 robust percentile 与极值来源。
3. 拒绝单个噪声点定义最大高度。
4. 输出 distribution summary 与 quality。

## 5. 算法/规则设计

**实施分类：** `SPI Metrology`

**Primary route：** `max/percentile/robust height metrics`

**Fallback route：** `Robust percentile/maximum metric per MeasurementSpec`

**Fallback trigger：** The statistic (max/percentile) is frozen by MeasurementSpec; it cannot be chosen by UI at runtime.
...
| Algorithm baseline | `max/percentile/robust height metrics` |
...
|---|---|---|
| Primary | `max/percentile/robust height metrics` | 仅在本 FS 的适用域、输入质量门和参数来源满足时执行。 |
| Fallback | `Robust percentile/maximum metric per MeasurementSpec` | The statistic (max/percentile) is frozen by MeasurementSpec; it cannot be chosen by UI at runtime. |
...
| Algorithm baseline | `max/percentile/robust height metrics` | 只能在明确的条件下切换到 Fallback；不得凭个人偏好替换。 |
...
| Primary algorithm/route | `max/percentile/robust height metrics` |
| Fallback | `Robust percentile/maximum metric per MeasurementSpec` |
| Fallback trigger | The statistic (max/percentile) is frozen by MeasurementSpec; it cannot be chosen by UI at runtime. |
