...
1. 解析 ViaTarget 与 LayerStack context。
2. 局部边缘/轮廓。
3. 拟合并做 residual/validity。
4. 转换至 layer reference frame。
5. 进入 Metrology Qualification context。
...
|---|---|---|
| T31-01 | contract/schema validation | expected assertion + evidence + trace; no silent pass |
| T31-02 | normal + boundary + reject | expected assertion + evidence + trace; no silent pass |
| T31-03 | replay/golden dataset | expected assertion + evidence + trace; no silent pass |
| T31-04 | fault injection | expected assertion + evidence + trace; no silent pass |
| T31-05 | performance benchmark | expected assertion + evidence + trace; no silent pass |
| T31-06 | real domain consumer | expected assertion + evidence + trace; no silent pass |
| T31-07 | hard-negative/false-positive set | expected assertion + evidence + trace; no silent pass |
