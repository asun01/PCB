...
重复测量、标准件、Bias/Linearity/Stability、分量模型。
...
1. 列出贡献项与相关性假设。
2. 将 Type A/Type B 分量转换为标准不确定度。
3. 组合得到 uc；扩展不确定度按批准覆盖因子 k 形成 U。
4. 将 U 与方法适用域、guard band、规格判定策略关联。
5. 资格报告保存原始研究数据和版本。
...
| 变化/故障源 | 必测处理 | 结果要求 |
|---|---|---|
| insufficient data | 注入/回放 insufficient data | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
| correlation unknown | 注入/回放 correlation unknown | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
| drift | 注入/回放 drift | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
| domain extrapolation | 注入/回放 domain extrapolation | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
