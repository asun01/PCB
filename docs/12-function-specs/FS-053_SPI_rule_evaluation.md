...
1. 校验 MeasurementQuality/coverage。
2. 读取已发布 MeasurementSpec/limits。
3. 执行 guard-band/判定逻辑。
4. 生成 RuleEvaluation 与 InspectionDecision proposal。
5. 生产 MachineDecision 必须由 Decisioning Owner 提交。
...
**Reject / Block conditions：** Missing rule, incompatible units, quality gate failure, unknown measurement.

**参数键（名称冻结，数值由批准 Profile / Recipe / Qualification 提供）：** `['RuleSetRevisionId', 'UnitCompatibilityPolicy', 'UnknownPolicy']`
...
| Reject / Block | `Missing rule, incompatible units, quality gate failure, unknown measurement.` | 保留证据与原因；不得生成权威 OK。 |
...
| Fallback trigger | Never replace a missing threshold/rule with a default. Rule version must be explicit. |
| Reject/Block | Missing rule, incompatible units, quality gate failure, unknown measurement. |
