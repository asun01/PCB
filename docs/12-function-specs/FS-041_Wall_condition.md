...
1. 获得局部 3D surface。
2. 建立 wall profile。
3. 测量 taper/roughness-like descriptors only when method qualified。
4. 检查缺口/污染。
5. 输出 maintenance-relevant facts。
...
**Reject / Block conditions：** Wall profile incomplete, residual above gate, surface occluded/invalid.
...
| Reject / Block | `Wall profile incomplete, residual above gate, surface occluded/invalid.` | 保留证据与原因；不得生成权威 OK。 |
...
| Fallback trigger | Fallback only between qualified profile sampling methods for the same wall-condition definition. |
| Reject/Block | Wall profile incomplete, residual above gate, surface occluded/invalid. |
