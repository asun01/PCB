...
1. 建立 Observation 集合并去重；保留原始观察和来源证据。
2. 按批准 IdentityPolicy 执行精确匹配、规范化匹配、优先级和冲突矩阵。
3. 产生 `IdentityResolutionReceipt`，说明 accepted/rejected/unresolved/conflict 和依据。
4. 只有 Orchestrator 身份 Owner 才能把解析结果绑定为 `BoardIdentity`。
5. 将结果绑定到 Recipe/Runtime admission 检查；未决身份走 retained/hold/recovery，而不是丢弃。
...
**参数键（名称冻结，数值由批准 Profile / Recipe / Qualification 提供）：** `['IdentityPolicyVersion', 'ObservationFreshnessWindow', 'SourceTrustClass', 'ScopeId']`
