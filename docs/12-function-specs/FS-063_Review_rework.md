...
输入契约：`BOM/PnP/Library/Image`；输出契约：`AOI facts/review/rework`。若字段尚未在现有 Schema 中冻结，必须登记到对应 ContractCandidate（通常 C-01/C-02/C-03/C-04/C-08），不能在功能目录中另建 JSON/数据库模型。
...
1. 加载 committed evidence。
2. 显示候选判定和依据。
3. ReviewDecision 独立记录。
4. ReworkOrder/RepairAction/Verification 独立关联。
5. 最终处置通过既有 owner。
...
**Reject / Block conditions：** Unauthorized reviewer, claim expired, evidence missing, disposition conflict.

**参数键（名称冻结，数值由批准 Profile / Recipe / Qualification 提供）：** `['ReviewClaimPolicyId', 'ClaimLease', 'DispositionPolicyId']`
...
| 变化/故障源 | 必测处理 | 结果要求 |
|---|---|---|
| 证据缺失 | 注入/回放 证据缺失 | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
| 重复复判 | 注入/回放 重复复判 | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
| 返修循环 | 注入/回放 返修循环 | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
| 权限不足 | 注入/回放 权限不足 | 显式质量状态；必要时 Reject/Review，不得静默 PASS |
...
Review workspace 默认“证据优先”：原图/3D、候选类别、依据、相似案例、操作历史、返修/复验入口并列；不可改写原机判事实。
...
| Reject / Block | `Unauthorized reviewer, claim expired, evidence missing, disposition conflict.` | 保留证据与原因；不得生成权威 OK。 |
...
| Implementation | `ReviewRework.cs` | Pipeline/algorithm implementation |
...
|---|---|---|
| T63-01 | contract/schema validation | expected assertion + evidence + trace; no silent pass |
| T63-02 | normal + boundary + reject | expected assertion + evidence + trace; no silent pass |
| T63-03 | replay/golden dataset | expected assertion + evidence + trace; no silent pass |
| T63-04 | fault injection | expected assertion + evidence + trace; no silent pass |
| T63-05 | performance benchmark | expected assertion + evidence + trace; no silent pass |
| T63-06 | real domain consumer | expected assertion + evidence + trace; no silent pass |
| T63-07 | hard-negative/false-positive set | expected assertion + evidence + trace; no silent pass |
...
| Fallback trigger | There is no automatic quality fallback. If evidence is insufficient, Review remains unresolved. Rework changes are commands with their own receipt/authorization. |
| Reject/Block | Unauthorized reviewer, claim expired, evidence missing, disposition conflict. |
...
| Result/evidence | ReviewDecision + rework/disposition evidence + input snapshot + algorithm/parameter version + quality-gate outcome |
