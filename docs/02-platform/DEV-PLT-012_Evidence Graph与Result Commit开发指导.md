...
- 适用范围：CAS、Evidence Manifest、Commit Receipt、回读与审计
- 直接依赖：`DEV-GOV-004`, `DEV-GOV-008`
- 主要输出：事实提交闭环、幂等提交和证据回读
...
`事实提交闭环、幂等提交和证据回读` 是本指导的稳定闭环。实现时必须优先建立接口和可回放测试，再接入真实设备/产品。
...
Evidence Graph 必须支持从 BoardRun/Fact/Decision/Review/Report 逆向找到原始 evidence。大图和 3D 数据推荐使用内容寻址 + 分块 + 元数据；历史生产结果必须能在未来无 UI 页面原样解释其版本闭包。


## 实施级关联功能

本指导的具体功能以以下实施规范为准：FS-004。每个 FS-xxx 都包含输入/输出、算法、鲁棒性、性能、UI、测试、验收和禁止实现。
