...
- 适用范围：板/Panel/连续卷料、多工位、多线程、背压
- 直接依赖：`DEV-GOV-008`
- 主要输出：PerformanceProfile、DAG critical path、有界队列、资源租约
...
`PerformanceProfile、DAG critical path、有界队列、资源租约` 是本指导的稳定闭环。实现时必须优先建立接口和可回放测试，再接入真实设备/产品。
...
板级链路采用 DAG，不以“一个板一个线程”作为默认模型。队列必须有上限并定义满载行为；大图采用 tile/pyramid/LOD；3D 采用 ROI 限制和中间结果生命周期；计算资源按能力租约，避免隐藏的全局争用。性能门槛写进 Profile，不写进 UI 魔法数字。


## 实施级关联功能

本指导的具体功能以以下实施规范为准：FS-006。每个 FS-xxx 都包含输入/输出、算法、鲁棒性、性能、UI、测试、验收和禁止实现。
