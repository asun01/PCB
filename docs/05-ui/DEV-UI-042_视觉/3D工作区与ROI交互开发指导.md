...
- 适用范围：2D/3D Viewport、ROI、Overlay、坐标、测量标注
- 直接依赖：`DEV-VIS-022`, `DEV-UI-041`
- 主要输出：IImageViewport、I3DViewport、IRoiOverlay、命中测试
...
Viewport 只消费 immutable frame/evidence/overlay DTO，不持有 HALCON/AsunImage 原生对象，不拥有生产 Decision。大图必须 tile/pyramid/LOD；Overlay 用独立坐标变换层；命中测试和显示矩阵必须可测试。
