...
- 适用范围：位置变换、拼接质量、重叠、误差传播
- 直接依赖：`DEV-GOV-008`
- 主要输出：FOV Position Graph、Transform Artifact、Residual、Seam Quality
...
建立 `FOV Position Graph`：节点 = FOV/机器姿态；边 = 重叠区约束；全局求解 = 从局部约束得到统一变换；输出 = transform + covariance/residual + validity mask。每个变换都带来源标定、有效域和版本。

不得直接“把相邻图片平移相加”作为生产坐标。局部误差会累积。跨 FOV 量测使用全局坐标之前必须验证最差位置残差，并按计量资格门禁确认。


## 实施级关联功能

本指导的具体功能以以下实施规范为准：FS-015。每个 FS-xxx 都包含输入/输出、算法、鲁棒性、性能、UI、测试、验收和禁止实现。
