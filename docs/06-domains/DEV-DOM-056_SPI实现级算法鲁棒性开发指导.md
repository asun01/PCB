...
- 直接依赖：`DEV-DOM-055`, `DEV-VIS-021`
- 主要输出：默认/Fallback/拒识、性能与质量评分
...
2. 局部参考面估计先于高度测量。
3. 先候选，再高精度测量。
4. 对小 Pad/细边界保留高分辨率路径。
5. 对异常 HeightField 产生拒识而不是强制输出。
6. 任何阈值自动初始化必须有样本依据并进入 Draft，而非运行中无审计自调。

重点 Benchmark：体积偏差、面积偏差、峰值高度稳定性、边界偏差、运行时间、异常数据拒识率。
