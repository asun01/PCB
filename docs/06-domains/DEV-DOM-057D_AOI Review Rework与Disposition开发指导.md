...
复判工作区优先展示证据和相似历史案例，再给出下一操作。ReviewClaim 是“我正在处理哪个事实”的认领，不是生产判定。返修必须形成 RepairAction 与 ReworkVerification；再检产生新的 Attempt/Fact，不修改旧 Fact。
...
本指导的具体功能以以下实施规范为准：FS-063。每个 FS-xxx 都包含输入/输出、算法、鲁棒性、性能、UI、测试、验收和禁止实现。
