...
- 适用范围：焊盘/钢网/板面参考与局部基准
- 直接依赖：`DEV-DOM-055A`; `DEV-DOM-054A`; `DEV-MET-031`
- 主要输出：Reference Surface/Plane、Pad Target、Stencil Target
...
参考面必须声明来源、估计区域、拟合方法、残差、有效域。对起伏板面/非平面区域必须支持局部 reference，而不是强制全板一个平面。

## 目标生成
ManufacturingGeometrySnapshot → Pad/Stencil opening → local coordinate → measurement ROI。缺失 CAD/钢网输入时必须显式记录覆盖缺口，不能静默猜测。
...
本指导的具体功能以以下实施规范为准：FS-045, FS-046。每个 FS-xxx 都包含输入/输出、算法、鲁棒性、性能、UI、测试、验收和禁止实现。
