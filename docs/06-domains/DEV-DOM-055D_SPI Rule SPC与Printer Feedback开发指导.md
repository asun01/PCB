...
- 主要输出：Defect facts、SPC points、受控 feedback
...
## 判定
规则引擎消费 MeasurementSet/Quality/Eligibility，不直接访问原始图像。

## SPC
统计必须固定 subgroup、分母、水位、异常/迟到数据口径。

## Printer Feedback
```text
SPI Advice
 → Feedback Command Proposal
 → Authority/Permission
 → Printer ACK
 → Applied/Effect Evidence
```
只有 Applied/Effect 证据到达后才可说反馈生效。
...
本指导的具体功能以以下实施规范为准：FS-053, FS-054, FS-055。每个 FS-xxx 都包含输入/输出、算法、鲁棒性、性能、UI、测试、验收和禁止实现。
