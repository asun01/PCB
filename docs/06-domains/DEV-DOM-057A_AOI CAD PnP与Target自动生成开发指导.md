...
制造输入经 BoardDataAdapter/ManufacturingGeometrySnapshot 规范化；BOM/PnP/PackageLibrary 形成候选目标。生成器只产生 Draft。目标分组、方法选择和覆盖候选要记录生成理由、来源版本和未解决项。
...
本指导的具体功能以以下实施规范为准：FS-056。每个 FS-xxx 都包含输入/输出、算法、鲁棒性、性能、UI、测试、验收和禁止实现。
