...
public static class ClientResultsHistoryAuthorityDisplaySmoke
{
    ...
    private static void HistoryDisplayCarriesQualityAuthority()
    {
        ...
        var item=ClientRunHistoryPresentationRuntime.CreateItem(snapshot.History.Entries.Single());
        Check(item.QualityFingerprint==snapshot.Quality.Fingerprint && item.QualityText.StartsWith("Quality "),
    ...
    private static void HistoryDisplayCarriesReplayAuthority()
    {
        ...
        var snapshot=workspace.Capture();
        var item=ClientRunHistoryPresentationRuntime.CreateItem(snapshot.History.Entries.Single());
        Check(item.ReplayText.StartsWith("Replay ") && item.ReplayText.Contains(snapshot.Replay!.ReplayFingerprint[..12]),
    ...
    private static void HistoryAuthorityRemainsImmutableAfterRecovery()
    {
        ...
        workspace.ExecuteAsync(ClientSimulationSessionFactory.CreateReleaseManifest()).AsTask().GetAwaiter().GetResult();
    ...
    private static ClientWorkspaceClientSnapshot CreateResultsProjection(ClientInspectionWorkspaceSnapshot snapshot) =>
    ...
    private static ClientInspectionWorkspace CreateFinalizedWorkspace()
    {
        var workspace=new ClientInspectionWorkspace(
            ...
            new System.Numerics.Vector2(64,64));
        workspace.QualityProviderCatalog.Register(ClientSimulationQualityRunProvider.CreateDefinition());
    ...
    {
        if(!condition)
            throw new InvalidOperationException("Results History authority display smoke failed: "+message);
    }
}
