...
public static class ClientInspectionExecutionAuthorizationSmoke
{
    ...
    {
        var ready=Create(ClientExecutionStatus.Ready,true,true,true);
        var stale=Create(ClientExecutionStatus.Completed,true,true,true);
        Check(ready.RunReadinessState==ClientRunReadinessState.Ready &&
              ready.CanRunInspection &&
              stale.RunReadinessState!=ClientRunReadinessState.Ready &&
              stale.CanRunInspection==false,
            "Machine-readable readiness and execution capability must agree, and stale routed capability must not bypass the readiness fence.");
    }
