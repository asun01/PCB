...
The shell now launches a vendor-neutral WPF `Bootstrap/MainWindow.xaml` workspace host and the repository gates validate its startup binding and stable AutomationIds. It intentionally contains no production commands, fact writes, HALCON objects, or DevExpress references. The host is a reversible bootstrap boundary until the relevant Application Port/PageContract and DevExpress environment gate are resolved.
